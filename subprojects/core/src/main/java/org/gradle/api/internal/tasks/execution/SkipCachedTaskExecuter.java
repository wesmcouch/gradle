                    ...
                    try {
                        ...
                        if (useVersion2) {
                            originMetadata = buildCacheV2.load(
                                buildCacheCommandFactoryV2.createLoad(cacheKey, outputProperties, task, taskProperties.getLocalStateFiles(), taskOutputsGenerationListener, taskState)
                            );
                        } else {
                            originMetadata = buildCache.load(
                                buildCacheCommandFactory.createLoad(cacheKey, outputProperties, task, taskProperties.getLocalStateFiles(), taskOutputsGenerationListener, taskState)
                            );
                        }
